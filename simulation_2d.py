...
def solve_velocity(tau, theta, sigma, a, b, L, mu_0, V_0, rad_damp):
    ...
    for i in range(n):
        ...
        V[i] = min(max(V_guess, 1e-20), 20.0) # Clip
...
def run_simulation(t_years: float = 1000, polygon_json_path: str = None):
    ...
    while t < t_end:
        # 速度計算
        V, success = solve_velocity(tau, theta, sigma, a, b, L, mu_0, V_0, rad_damp)
        if not success:
             # 重度でなければ続行するが、警告を出す
             if step % 1000 == 0:
        ...
        # 提案dt
        if V_max > V_SEISMIC:
            dt_proposal = min(1.0, L_min / V_max / 10)
            dt_proposal = max(dt_proposal, 0.01)
        elif V_max > V_PRESEISMIC:
            dt_proposal = min(3600.0, L_min / V_max / 20)
            dt_proposal = max(dt_proposal, 1.0)
        else:
            ...
            if max_dtheta_ratio > 0:
                dt_theta = 0.1 / max_dtheta_ratio
            else:
                dt_theta = 86400.0 * 30
            dt_proposal = min(86400.0 * 30, dt_theta)
            dt_proposal = max(dt_proposal, 60.0)

        if step > 0:
            dt_proposal = min(dt_proposal, dt_prev * 1.5)

        dt = max(dt_proposal, 0.001)
        ...
        # ここではハードクリッピングは削除し、リトライロジックに任せる
        # ただし最後の安全装置としてクリッピングを緩くかける（数値爆発防止）
        max_dtau = 0.5 * np.min(sigma)
        dtau_step = np.clip(dtau_step, -max_dtau, max_dtau)
